...
module.exports = async callback => {
  try {
    const nfty = await Temp.deployed()

    const tokenAddress = await nfty.LinkToken()
    console.log("Chainlink Token Address: ", tokenAddress)
    const token = await LinkTokenInterface.at(tokenAddress)
    console.log('Funding contract:', nfty.address)
    const tx = await token.transfer(nfty.address, payment)
    callback(tx.tx)
  } catch (err) {
    callback(err)
  }
